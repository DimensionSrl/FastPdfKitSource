...
-(void)selectorWholeTextForPage:(NSNumber *)page {
    ...
    @autoreleasepool {
        ...
        // Just call the -wholeTextForPage: method of MFDocumentManager. Pass
        // NULL as profile to use the default profile.
        // If you want to use a different profile pass a reference to a
        // MFProfile.
        
        // Use -(void)test_wholeTextForPage:(NSUInteger)page if you want to test
        // the new text extraction engine instead.
        NSString *someText = nil;
        NSUInteger pages = [documentManager numberOfPages];
        NSUInteger iterations = 0;
        NSUInteger maxIterations = 1000;
        
        NSUInteger pageToSearch = [page intValue];
        while(iterations++ < maxIterations)
        {
            pageToSearch = (pageToSearch + 1) % pages;
            someText = [[documentManager wholeTextForPage:pageToSearch]copy];
            NSLog(@"Page %d iteration %d", pageToSearch, iterations);
        }
    ...
    NSString * someText = [documentManager wholeTextForPage:page.integerValue];
